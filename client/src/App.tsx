...
import "./index.css";
import { getLocaleFromPath, getRoutePath, localizedPath, useSiteSeo } from "./seo";
...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  const routePath = getRoutePath(rawLocation);
  const pathLocale = getLocaleFromPath(rawLocation);
  ...
  const routeLabel = routePath === "/" ? labels.home : (() => { const entry = menu.find(([href]) => href === routePath); return entry ? localizedMenu[locale][entry[1]] : routePath.slice(1).toUpperCase(); })();
  useEffect(() => {
    if (pathLocale && pathLocale !== locale) setLocale(pathLocale);
  }, [pathLocale, locale]);
  useSiteSeo(locale, rawLocation);
  useEffect(() => {
  ...
  return (
    <div className={`kyara-app ${routePath === "/mente" ? "is-mente" : ""}`}>
      ...
      {hasStarted && <header className="site-header">
        <Link href={localizedPath("/", locale)} className="wordmark glitch-text" aria-label={labels.wordmark}>
        ...
        <label className="language-control" aria-label={labels.selectLanguage}>
          ...
          <select value={locale} onChange={(event) => { const nextLocale = event.target.value as Locale; setLocale(nextLocale); navigate(localizedPath(routePath, nextLocale)); }}>
      ...
      {hasStarted && <>
        ...
        <LocaleContext.Provider value={locale}><main onClickCapture={openMediaLightbox}><div key={rawLocation} className="route-view">{children}</div></main></LocaleContext.Provider>
  ...
  return (
    <div className="nav-overlay" role="dialog" aria-modal="true" aria-label={labels.mainMenu}>
      ...
      <div className="nav-list">
        {menu.map(([href, label, number]) => <Link key={href} href={localizedPath(href, locale)} className="nav-item">
...
function CTA({ href, children, violet = false }: { href: string; children: React.ReactNode; violet?: boolean }) { const locale = useContext(LocaleContext); return <Link href={localizedPath(href, locale)} className={`cta ${violet ? "violet" : ""}`}>{children}<ArrowRight size={16} /></Link>; }
...
function Home() {
  ...
  const openEgg = () => {
    ...
    if (!audio) return;
    audio.currentTime = 0;
    audio.play().catch(() => toast.info(visual.playToast));
  };
  ...
  return <>
    <section className="hero" data-media-src={A.hero} data-media-type="image" data-media-alt="KYARA NOVA" style={{ backgroundImage: `linear-gradient(90deg, rgba(5,7,8,.92) 0%, rgba(5,7,8,.55) 48%, rgba(5,7,8,.18) 100%), url(${A.hero})` }}>
      <div className="hero-inner">
        <div className="hero-copy">
          ...
          <h1>{visual.heroA}<br /><em>{visual.heroB}</em></h1>
        ...
        <div className="hero-readout">
          <div className="readout-line"><span>{visual.source}</span><b>{visual.unknown}</b></div><div className="readout-line"><span>FREQ</span><b>1420.405751 MHz</b></div><div className="readout-line"><span>CONFIDENCE</span><b>97.2%</b></div>
          <div className="signal-bars"><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/></div>
          <span className="readout-note">{visual.listeningInto}<br />{visual.unknown}</span>
        </div>
      </div>
      <div className="hero-bottom"><span>{visual.scrollToReceive}</span><div className="oscilloscope"><i/><i/><i/><i/><i/><i/><i/></div><span>01 / 07</span></div>
    </section>
    <section className="intro-band section-shell"><div className="section-marker">01 <span>/ {visual.forest}</span></div><div className="intro-grid"><div><h2>{copy.forestHeadingA}<br /><em>{copy.forestHeadingB}</em></h2></div><div className="body-copy"><p>{copy.forestBody}</p><p>{copy.manifestoLead}</p><CTA href="/kyaraverse">{copy.discoverOrigin}</CTA></div></div></section>
    ...
    <section className="manifesto-band section-shell"><div className="section-marker">01 <span>/ {visual.traces}</span></div><div className="manifesto-content"><h2>{homeText.tracesA}<br /><em>{homeText.tracesB}</em></h2><p className="body-copy">{homeText.tracesBody}</p><p className="body-copy">{copy.manifestoLead}</p><a className="cta" href="https://www.instagram.com/just.k.nova" target="_blank" rel="noreferrer">{copy.discoverOrigin} <ArrowUpRightIcon /></a></div></section>
    <section className="terminal-section section-shell" id="terminal"><div className="section-marker">02 <span>/ {visual.interception}</span></div><div className="terminal-intro"><div><h2>{homeText.terminalA}<br /><em>{homeText.terminalB}</em></h2><p className="body-copy">{homeText.terminalBody}</p></div><div className="terminal-side-note"><Binary size={22} /><span>{visual.inputListening}</span></div></div><Terminal /></section>
    <ArchiveSection />
    <section className="decoder-band section-shell"><div className="section-marker">03 <span>/ {visual.accessCodex}</span></div><div className="decoder-grid"><div><Eyebrow>{visual.fragmentFound}</Eyebrow><h2>{homeText.codexA}<br /><em>{homeText.codexB}</em></h2><p className="body-copy">{homeText.codexBody}</p></div><CodexDecoder /></div></section>
    <section className="lunar-section"><div className="lunar-inner"><div><Eyebrow>{homeArtifact.signalState}</Eyebrow><h2>{homeText.lunarA}<br /><em>{homeText.lunarB}</em></h2><p>{homeText.lunarBody}</p></div><div className="moon-readout"><Moon className="signal-pulse-moon" size={72} strokeWidth={1} /><span>57.4%</span><small>{visual.illumination}<br />{visual.currentCycle}</small></div></div></section>
    <section className="final-cta section-shell">
        ...
        {eggOpen && (
          <div className="egg-popover" role="dialog" aria-modal="false" aria-label={labels.eggDialog}>
            ...
            <div className="egg-system-screen">
              <span className="egg-system-label">{visual.systemMessage}</span>
              <p>{eggText}</p>
            </div>
            <div className="egg-modal-actions">
              <button className="egg-audio-btn" onClick={() => {
                const audio = eggAudioRef.current;
                if (!audio) return;
                if (audio.paused) audio.play().catch(() => toast.info(visual.playToast));
                else audio.pause();
              }}>
      ...
      <div><Eyebrow>{visual.transmissionContinues}</Eyebrow><h2>{homeText.finalA}<br /><em>{homeText.finalB}</em></h2><CTA href="/mural" violet>{homeText.finalCta}</CTA></div>
    </section>
    <section className="home-final-gallery section-shell" aria-label={visual.visualArchive}>
      <button type="button" className="home-final-photo" data-media-src={A.homeMoonStage} data-media-type="image" data-media-alt={visual.moonAlt} aria-label={visual.moonAlt}><img src={A.homeMoonStage} alt={visual.moonAlt.replace(/^Open image of |^Abrir imagem de |^Ouvrir l'image de |^달 아래 |^打开月下 /, "")} loading="eager" decoding="async" fetchPriority="low" /></button>
      <button type="button" className="home-final-photo" data-media-src={A.homeCrystalHalo} data-media-type="image" data-media-alt={visual.crystalAlt} aria-label={visual.crystalAlt}><img src={A.homeCrystalHalo} alt={visual.crystalAlt.replace(/^Open image of |^Abrir imagem de |^Ouvrir l'image de |^수정 사이의 |^打开水晶之间的 /, "")} loading="eager" decoding="async" fetchPriority="low" /></button>
    </section>
  </>;
}
...
function ArchiveVideoCard({ video }: { video: { src: string; poster: string; title: string; meta: string } }) {
  ...
  const togglePlay = () => {
    ...
    if (!element) return;
    if (element.paused) element.play().then(() => setPlaying(true)).catch(() => toast.info(visual.playToast));
...
function CodexDecoder() {
  ...
  const decode = () => { setDecoding(true); setDecoded(""); setTimeout(() => { const result = value.split(" ").map((x) => { const n = parseInt(x, 2); return Number.isNaN(n) ? "" : String.fromCharCode(n); }).join(""); setDecoded(result || visual.invalidSignal); setDecoding(false); toast.success(visual.decodedToast); }, 850); };
  return <div className="codex-box"><div className="codex-top"><span><Binary size={16} /> CODEX_DECODER</span><span>01 / 02</span></div><textarea aria-label={labels.binaryCode} value={value} onChange={(e) => setValue(e.target.value)} spellCheck={false} /><div className="codex-actions"><button className="decode-button" onClick={decode}>{decoding ? labels.decoding : labels.decode}<ArrowRight size={16} /></button><span>ASCII / BINARY</span></div>{decoded && <div className="decoded-output"><Sparkles size={16} /><span>{decoded}</span><b>{visual.crystals}</b></div>}</div>;
}

function HeroArtifact({ kind }: { kind: "lore" | "entity" | "lexicon" | "catalog" | "channel" }) {
  const locale = useContext(LocaleContext);
  const visual = visualLabels[locale];
  const data = { lore: { label: "MYTH TIMELINE", code: "NOVA // 05 CHAPTERS" }, entity: { label: "ENTITY DOSSIER", code: "KYARA // 001" }, lexicon: { label: "INDEX SCANNER", code: "GLOSS // 07 ENTRIES" }, catalog: { label: "ARTIFACT CATALOG", code: "STORE // ACTIVE" }, channel: { label: "OPEN FREQUENCY", code: "MURAL // 200 OK" } }[kind];
  return <div className={`hero-artifact artifact-${kind}`}><div className="artifact-orbit"><span className="artifact-glyph">{kind === "channel" ? "〰" : kind === "lexicon" ? "≡" : kind === "catalog" ? "◇" : kind === "entity" ? "◎" : "✦"}</span></div><div className="artifact-readout"><span>{data.label}</span><b>{data.code}</b><i /></div><small>{visual.scrollToReceive}</small></div>;
}

function PageHero({ eyebrow, title, subtitle, accent = "cyan", image, kind: kindOverride }: { eyebrow: string; title: React.ReactNode; subtitle: string; accent?: "cyan" | "violet" | "red"; image?: string; kind?: "lore" | "entity" | "lexicon" | "catalog" | "channel" }) {
  const locale = useContext(LocaleContext);
  ...
  const kind = kindOverride ?? (eyebrow.includes("LORE") ? "lore" : eyebrow.includes("ENTIDADE") ? "entity" : eyebrow.includes("LÉXICO") ? "lexicon" : eyebrow.includes("COMÉRCIO") ? "catalog" : "channel");
...
function Kyaraverse() {
  ...
  return (
    <>
      <PageHero eyebrow={visual.loreProtocol} kind="lore" title={<>{copy.kyaraverseTitleA}<br /><em>{copy.kyaraverseTitleB}</em></>} subtitle={copy.kyaraverseSubtitle} image={A.aether} />
      ...
      <div className="lore-layout">
        <aside className="lore-index"><span>{visual.loreIndex}</span>{localizedChapters.map((c) => <a key={c.id} href={`#${c.id}`}>{c.no}</a>)}</aside>
      ...
      <section className="rules-section section-shell">
        <Eyebrow>{visual.rulesLabel}</Eyebrow>
...
function Kyara() {
  ...
  return (
    <>
      <PageHero eyebrow={visual.entityIdentity} kind="entity" title={<>{copy.kyaraTitleA}<br /><em>{copy.kyaraTitleB}</em></>} subtitle={copy.kyaraSubtitle} accent="violet" />
      <section className={`kyara-profile section-shell ${mode}`}>
        <div className="profile-visual">
          <img src={mode === "natural" ? A.portrait : A.full} alt={visual.kyaraAlt} loading="lazy" decoding="async" />
          <div className="profile-stamp"><span>KYARA</span><b>NOVA</b><small>ENTITY 001</small></div>
        </div>
        <div className="profile-copy">
          <Eyebrow>{mode === "natural" ? visual.naturalListening : visual.performanceTransmitting}</Eyebrow>
          <h2>{copy.artistHeadingA}<br /><em>{copy.artistHeadingB}</em></h2>
          <p>{copy.artistBody}</p>
          <p className="body-copy">{visual.aiPerspective}</p>
...
function RouteContent() {
  ...
  const routePath = getRoutePath(rawLocation);
  return <Switch location={routePath}><Route path="/" component={Home} /><Route path="/kyaraverse" component={Kyaraverse} /><Route path="/kyara" component={Kyara} /><Route path="/discografia" component={Discografia} /><Route path="/glossario" component={Glossario} /><Route path="/shopping" component={Shopping} /><Route path="/mural/inbox" component={MuralInbox} /><Route path="/mural" component={Mural} /><Route path="/mente" component={Mente} /><Route component={NotFound} /></Switch>;
}

export default function App() { return <><Toaster theme="dark" position="bottom-right" /><Shell><Suspense fallback={<div className="route-view" aria-busy="true" />}><RouteContent /></Suspense></Shell></>; }
